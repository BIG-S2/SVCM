findVoxelSequence<-function(ch, S, dimn)
{
  
  mask=array(rep(1,prod(dimn)),dimn);
  maskVector = c(mask);
  nonzeroId = which(maskVector!=0);
  nId = length(nonzeroId); 
  yy=xx=zz=array(rep(1,64*64*8),c(64,64,8));
  for(i in 1:8)
  {
    xx[,,i]=meshgrid(1:64, 1:64)$y;
    yy[,,i]=meshgrid(1:64, 1:64)$x;
    zz[,,i]=i*zz[,,i];
  }
  xx = xx[nonzeroId]; 
  yy = yy[nonzeroId];
  ...
  loc = match(subVSeq,nonzeroId);
  loc[is.na(loc)]=0
  VSeqId[iId,1:InVSeq[iId]] = t(loc);
  }
